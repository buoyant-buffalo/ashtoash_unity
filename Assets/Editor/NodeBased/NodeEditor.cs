...
public class NodeEditor : EditorWindow {
		...
		if(e.button == 1 && !makeTransitionMode)
		{
			if(e.type == EventType.MouseDown)
			{
				...
				if(!clickedOnWindow)
				{
                    ...
                    menu.AddItem(new GUIContent("Add FullTree Node"), false, ContextCallback, "fullTree");
                    menu.AddItem(new GUIContent("Add Branch Node"), false, ContextCallback, "branchNode");
                    menu.AddItem(new GUIContent("Text Node"), false, ContextCallback, "textNode");
	...
	void ContextCallback(object obj)
	{
        ...
        else if (clb.Equals("fullTree"))
        {
           
            FullTreeNode fullTreeNode = new FullTreeNode();
            fullTreeNode.windowRect = new Rect(mousePos.x, mousePos.y, 200, 100);

            windows.Add(fullTreeNode);
        }
        else if (clb.Equals("branchNode"))
        {
            globalID += 1;
            BranchNode branchNode = new BranchNode(globalID);
            branchNode.windowRect = new Rect(mousePos.x, mousePos.y, 100, 100);

            windows.Add(branchNode);
        }
        else if (clb.Equals("textNode"))
        {
            globalID += 1;
            TextNode textNode = new TextNode(globalID);
            textNode.windowRect = new Rect(mousePos.x, mousePos.y, 130, 130);

            windows.Add(textNode);
        }
		...
		{   
            //update global id?
            //should I? probalby, do i want to? not really
			bool clickedOnWindow = false;
			int selectIndex = -1;
            ...
            for (int i = 0; i < windows.Count; i++)
            {
                GameObject go = new GameObject("MyCreatedGo" + windows[i].numberID);
